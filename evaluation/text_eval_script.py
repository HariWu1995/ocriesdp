...
def validate_data(gtFilePath, submFilePath, evaluationParams):
    ...
    for k in gt:
        rrc_evaluation_funcs.validate_lines_in_file_gt(k, gt[k], evaluationParams['CRLF'],
    ...
    for k in subm:
        ...
        rrc_evaluation_funcs.validate_lines_in_file(k, subm[k], evaluationParams['CRLF'],
                                                                evaluationParams['LTRB'], True, 
...
def evaluate_method(gtFilePath, submFilePath, evaluationParams):
    ...
    for resFile in gt:
        ...
        if resFile in subm:
            ...
            pointsList, _, \
            transcriptionsList = rrc_evaluation_funcs.get_tl_line_values_from_file_contents_det(detFile, evaluationParams['CRLF'],
                                                                                                         evaluationParams['LTRB'], True,
                                                                                                         evaluationParams['CONFIDENCES'])
            for n in range(len(pointsList)):
                points = pointsList[n]
                    ...
                    for detNum in range(len(detPols)):
                        ...
                        and detRectMat[detNum] == 0 and detNum not in detDontCarePolsNum :
                            if iouMat[gtNum, detNum] > evaluationParams['IOU_CONSTRAINT']:
                                ...
                                # detOnlyCorrect += 1
                                if evaluationParams['WORD_SPOTTING']:
                                    edd = string_metric.levenshtein(gtTrans[gtNum].upper(), 
