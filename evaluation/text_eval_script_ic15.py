...
from Polygon.Shapes import Polygon
import numpy as np

from evaluation import rrc_evaluation_funcs_ic15 as rrc_evaluation_funcs
...
def validate_data(gtFilePath, submFilePath, evaluationParams):
    ...
    Method validate_data: 
        Validates that all files in the results folder are correct (have the correct name contents).
        Validates also that there are no missing files in the folder.
        If some error detected, the method raises the error.
    """
    gt   = rrc_evaluation_funcs.load_zip_file(  gtFilePath, evaluationParams['GT_SAMPLE_NAME_2_ID'], False)
...
def evaluate_method(gtFilePath, submFilePath, evaluationParams):
    ...
    def include_in_dictionary(transcription):
        ...
        if transcription[len(transcription)-2:] == "'s" \
    ...
    for resFile in gt:
        ...
        pointsList, _, \
        transcriptionsList = rrc_evaluation_funcs.get_tl_line_values_from_file_contents(gtFile, evaluationParams['CRLF'], 
        ...
        if resFile in subm:
            ...
            pointsList, confidencesList, \
                transcriptionsList = rrc_evaluation_funcs.get_tl_line_values_from_file_contents(detFile, evaluationParams['CRLF'],
                                                                                                         evaluationParams['LTRB'], True,
            ...
            if len(gtPols) > 0 and len(detPols) > 0:
                ...
                for gtNum in range(len(gtPols)):
                    for detNum in range(len(detPols)):
                        ...
                        and detRectMat[detNum] == 0 and detNum not in detDontCarePolsNum :
                            if iouMat[gtNum,detNum] > evaluationParams['IOU_CONSTRAINT']:
                                ...
                                if evaluationParams['WORD_SPOTTING']:
                                    ...
                                    correct = transcription_match(gtTrans[gtNum].upper(), 
                                                                detTrans[detNum].upper(), evaluationParams['SPECIAL_CHARACTERS'],
                                                                                          evaluationParams['ONLY_REMOVE_FIRST_LAST_CHARACTER']) == True
                ...
                for gtNum in range(len(gtPols)):
                    for detNum in range(len(detPols)):
                        if   det_only_gtRectMat[ gtNum] == 0 and  gtNum not in  det_only_gtDontCarePolsNum \
                        and det_only_detRectMat[detNum] == 0 and detNum not in det_only_detDontCarePolsNum:
                            if iouMat[gtNum, detNum] > evaluationParams['IOU_CONSTRAINT']:
        ...
        det_only_hmean = 0 if (det_only_precision + det_only_recall) == 0 \
    ...
    methodHmean = 0 if methodRecall + methodPrecision == 0 \
    ...
    det_only_methodHmean = 0 if det_only_methodRecall + det_only_methodPrecision == 0 \
                        else 2 * det_only_methodRecall * det_only_methodPrecision / (det_only_methodRecall + det_only_methodPrecision)

    methodMetrics          =            f"E2E_RESULTS: precision: {         methodPrecision}, recall: {         methodRecall}, hmean: {         methodHmean}"
    det_only_methodMetrics = f"DETECTION_ONLY_RESULTS: precision: {det_only_methodPrecision}, recall: {det_only_methodRecall}, hmean: {det_only_methodHmean}"
