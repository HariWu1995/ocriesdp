...
def weighted_edit_distance(word1: str, word2: str, scores: np.ndarray, ct_labels_inv):
    ...
    for i in range(1, n+1): 
        for j in range(1, m+1):
            delect_cost = _ed_delete_cost(j-1, i-1, word1, word2, scores, ct_labels_inv)  ## delete a[i]
            ...
            if word1[j-1] != word2[i-1]:
                ...
                replace_cost = 0
            dp[i][j] = min(dp[i-1][j  ] +  insert_cost, 
                           dp[i  ][j-1] +  delect_cost, 
...
def _ed_insert_cost(i, j, word1, word2, scores, ct_labels_inv):
    ...
    if i < len(word1) - 1:
        return (_get_score(scores[i  ], word1[i  ], ct_labels_inv) + 
                _get_score(scores[i+1], word1[i+1], ct_labels_inv)) / 2
    else:
        return _get_score(scores[i], word1[i], ct_labels_inv)
...
def _ed_replace_cost(i, j, word1, word2, scores, ct_labels_inv):
    ...
    return max(1 - _get_score(scores[i], word2[j], ct_labels_inv) / \
                   _get_score(scores[i], word1[i], ct_labels_inv) * 5, 0)
...
class LexiconMatcher:
    ...
    def _load_lexicon(self, dataset, lexicon_type):
        ...
        g_lexicon_path, g_pairlist_path, w_lexicon_path, w_pairlist_path, s_lexicon_path, s_pairlist_path = self._get_lexicon_path(dataset)
