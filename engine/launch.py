...
from utils.comm.multi_gpu import synchronize, _LOCAL_PROCESS_GROUP
...
def launch(main_func, num_gpus_per_machine, num_machines=1, machine_rank=0,
    ...
    if world_size > 1:
        ...
        mp.spawn(_distributed_worker, nprocs=num_gpus_per_machine, args=(main_func, world_size,
                                                                         num_gpus_per_machine, machine_rank, 
                                                                         dist_url, args, timeout,), 
                daemon=False,)
    else:
        main_func(*args)
