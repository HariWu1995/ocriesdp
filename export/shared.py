        ...
        if input.dim() == 4:
            ...
            if mode == "nearest":
                ...
                logger.warning(
                    ...
                    "because there's no such C2 op, this may cause significant slowdown "
                    "and the boundary pixels won't be as same as using F.interpolate due to padding."
                )
...
def create_const_fill_op(name: str, blob: Union[np.ndarray, workspace.Int8Tensor],
                        device_option: Optional[caffe2_pb2.DeviceOption] = None,) -> caffe2_pb2.OperatorDef:
    """
...
def construct_init_net_from_params(params: Dict[str, Any], 
                  device_options: Optional[Dict[str, caffe2_pb2.DeviceOption]] = None) -> caffe2_pb2.NetDef:
    """
...
def get_params_from_init_net(init_net: caffe2_pb2.NetDef,) -> Any[Dict[str, Any], Dict[str, caffe2_pb2.DeviceOption]]:
    """
...
def _generic_status_identifier(predict_net: caffe2_pb2.NetDef, status_updater: Callable,
                                known_status: Dict[Tuple[str, int], Any],) -> Dict[Tuple[str, int], Any]:
    """
    ...
    def _check_and_update(key, value):
        ...
        if key in _known_status:
            if not _known_status[key] == value:
                raise RuntimeError(
                    f"Confilict status for {key}, existing status {_known_status[key]}, new status {value}"
                )
...
def infer_device_type(predict_net: caffe2_pb2.NetDef, known_status: Dict[Tuple[str, int], Any],
                device_name_style: str = "caffe2",) -> Dict[Tuple[str, int], str]:
    """ 
...
def get_sub_graph_external_input_output(predict_net: caffe2_pb2.NetDef, 
                                sub_graph_op_indices: List[int]) -> Tuple[List[Tuple[str, int]], List[Tuple[str, int]]]:
    """
