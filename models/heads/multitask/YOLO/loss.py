...
class BCEBlurWithLogitsLoss(nn.Module):
    ...
    def forward(self, pred, true):
        ...
        alpha_factor = 1 - torch.exp((dx - 1) / (self.alpha + EPS))
        loss *= alpha_factor
        return loss.mean()
...
class SigmoidBin(nn.Module):
    ...
    def __init__(self, bin_count=10, min=0.0, max=1.0, reg_scale = 2.0, use_loss_regression=True, use_fw_regression=True, BCE_weight=1.0, smooth_eps=0.0):
        ...
        bins = torch.range(start, end+EPS, step).float() 
...
class FocalLoss(nn.Module):
    """
    Wraps focal loss around existing loss_func()
    
    criteria = FocalLoss(nn.BCEWithLogitsLoss(), gamma=1.5)
    """
    def __init__(self, loss_func=None, gamma=1.5, alpha=0.25):
        super(FocalLoss, self).__init__()
        self.loss_func = nn.BCEWithLogitsLoss()
        self.gamma = gamma
        self.alpha = alpha
        self.reduction = self.loss_func.reduction
        self.loss_func.reduction = 'none'  # required to apply FL to each element
...
class QFocalLoss(nn.Module):
    """
    Wraps Quality focal loss around existing loss_func()
...
class aLRPLoss(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx, logits, targets, regression_losses, delta=1.): 
        classification_grads = torch.zeros(logits.shape).cuda()
...
class ComputeLossOTA:
    ...
    def build_targets(self, p, targets, imgs):
        ...
        for batch_idx in range(p[0].shape[0]):
            ...
            pair_wise_iou_loss = -torch.log(pair_wise_iou + EPS)

            top_k, _ = torch.topk(pair_wise_iou, min(10, pair_wise_iou.shape[1]), dim=1)
            dynamic_ks = torch.clamp(top_k.sum(1).int(), min=1)

            gt_cls_per_image = F.one_hot(this_target[:, 1].to(torch.int64), self.nc).float()\
                                                                                    .unsqueeze(1)\
                                                                                    .repeat(1, pxyxys.shape[0], 1)

            num_gt = this_target.shape[0]
            cls_preds_ = p_cls.float().unsqueeze(0).repeat(num_gt, 1, 1).sigmoid_() \
                       * p_obj.float().unsqueeze(0).repeat(num_gt, 1, 1).sigmoid_()


            y = cls_preds_.sqrt_()
            pair_wise_cls_loss = F.binary_cross_entropy_with_logits(torch.log(y/(1-y)), 
                                                                    gt_cls_per_image, 
