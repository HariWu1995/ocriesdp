...
from models.layers.transformation.encodings import PositionalEncoding2D
from models.heads.testr.loss import SetCriterion
from models.heads.testr.matcher import build_matcher
from models.heads.testr.model import TESTR

from models.metrics import box_xyxy_to_cxcywh
from utils.batch import BatchedTensor
...
class Joiner(nn.Sequential):
    ...
    def forward(self, tensor_list: BatchedTensor):
        xs = self[0](tensor_list)
        out: List[BatchedTensor] = []
        pos = []
        for _, x in xs.items():
            out.append(x)
            # position encoding
...
class MaskedBackbone(nn.Module):
    """ 
    This is a thin wrapper around Detectron2's backbone to provide padding masking
    """
    ...
    def forward(self, images):
        ...
        for i, k in enumerate(features.keys()):
            features[k] = BatchedTensor(features[k], masks[i])
        return features
    ...
    def mask_out_padding(self, feature_shapes, image_sizes, device):
        ...
        for idx, shape in enumerate(feature_shapes):
            ...
            for img_idx, (h, w) in enumerate(image_sizes):
                masks_per_feature_level[img_idx, :int(np.ceil(float(h) / self.feature_strides[idx])),
...
class TransformerDetector(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        num_steps        = cfg.MODEL.TRANSFORMER.HIDDEN_DIM // 2
        self.use_polygon = cfg.MODEL.TRANSFORMER.USE_POLYGON
        self.threshold   = cfg.MODEL.TRANSFORMER.INFERENCE_THRESHOLD
        backbone = Joiner(d2_backbone, PositionalEncoding2D(num_steps, normalize=True))
        ...
        if loss_cfg.AUX_LOSS:
            ...
            for i in range(cfg.MODEL.TRANSFORMER.DEC_LAYERS - 1):
                aux_weight_dict.update({f'{k}_{i}': v for k, v in dec_weight_dict.items()})

            # encoder aux loss
            aux_weight_dict.update({f'{k}_enc': v for k, v in enc_weight_dict.items()})
            dec_weight_dict.update(aux_weight_dict)
        ...
        self.criterion = SetCriterion(self.testr.num_classes, box_matcher, point_matcher,
                                      dec_weight_dict, enc_losses, dec_losses, self.testr.num_ctrl_points, 
    ...
    def forward(self, batched_inputs):
        ...
        if self.training:
            ...
            ctrl_point_class = output["pred_logits"]
            ctrl_point_coord = output["pred_points"]
            text_pred        = output["pred_texts"]
            results = self.inference(ctrl_point_class, ctrl_point_coord, text_pred, images.image_sizes)
            processed_results = []
    ...
    def inference(self, ctrl_point_cls, ctrl_point_coord, text_pred, image_sizes):
        ...
        for scores_per_image, labels_per_image, ctrl_point_per_image, text_per_image, image_size in zip(
            ...
            selector = scores_per_image >= self.threshold
