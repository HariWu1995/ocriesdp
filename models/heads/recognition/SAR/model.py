...
class SAREncoder(nn.Layer):
    ...
    def forward(self, feat, img_metas=None):
        ...
        if valid_ratios is not None:
            ...
            valid_hf = holistic_feat[:, -1, :]  # B * C

        holistic_feat = self.linear(valid_hf)  # B * C
...
class ParallelSARDecoder(BaseDecoder):
    ...
    def __init__(self, out_channels,  # 90 + unknown + start + padding
                    enc_bi_rnn=False,
                    dec_bi_rnn=False, dec_drop_rnn=0.0, dec_gru=False,
                    d_model=512, d_enc=512, d_k=64,
                    pred_dropout=0.1, max_text_length=30, mask=True, pred_concat=True, **kwargs):
        super().__init__()
    ...
    def _2d_attention(self, decoder_input, feat, holistic_feat, valid_ratios=None):
        ...
        attn_weight = torch.permute(attn_weight, dims=[0, 1, 3, 4, 2])  # bsz * (seq_len + 1) * attn_size * h * w
        attn_weight = self.conv1x1_2(attn_weight)                       # bsz * (seq_len + 1) * h * w * attn_size
        ...
        attn_weight = torch.permute(attn_weight, dims=[0, 1, 4, 2, 3])
    ...
    def forward_test(self, feat, out_enc, img_metas):
        ...
        B = feat.shape[0]
        start_token = torch.full((B,), fill_value=self.start_idx, dtype='int64')
        start_token = self.embedding(start_token)

        emb_dim     = start_token.size()[1]
        start_token = start_token.unsqueeze(1)
        start_token = start_token.expand([B, seq_len, emb_dim])
        # bsz * seq_len * emb_dim
        out_enc = out_enc.unsqueeze(1)
        # bsz * 1 * emb_dim
...
class SARHead(nn.Layer):
    ...
    """
    def __init__(self, in_channels, out_channels, max_text_length=30,
                 enc_dim=512, enc_bi_rnn=False, enc_drop_rnn=0.1, enc_gru=False,
                 dec_bi_rnn=False, dec_drop_rnn=0.0, dec_gru=False,
                 d_k=512, pred_dropout=0.1, pred_concat=True, **kwargs):
