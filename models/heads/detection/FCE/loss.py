...
class FCELoss(nn.Module):
    ...
    num_samples (int) : The sampling points number of regression loss. 
        If it is too small, FCENet tends to be overfitting.
    ohem_ratio (float): the negative / positive ratio in OHEM.
    """
    def __init__(self, fourier_degree, num_samples, ohem_ratio=3.):
        super().__init__()
    ...
    def forward(self, preds, labels):
        ...
        loss_tr = torch.tensor(0., dtype='float32')
        loss_tcl = torch.tensor(0., dtype='float32')
        loss_reg_x = torch.tensor(0., dtype='float32')
        loss_reg_y = torch.tensor(0., dtype='float32')
        loss_all = torch.tensor(0., dtype='float32')
        ...
        results = dict(loss=loss_all, loss_text=loss_tr, loss_center=loss_tcl,
                                    loss_reg_x=loss_reg_x, loss_reg_y=loss_reg_y, )
        return results
    ...
    def forward_single(self, pred, gt):
        ...
        # tcl loss
        loss_tcl = torch.tensor(0., dtype='float32')
        tr_neg_mask = torch.logical_not(tr_train_mask)
        tr_neg_mask2 = torch.cat([tr_neg_mask.unsqueeze(1), 
                                  tr_neg_mask.unsqueeze(1)], dim=1)
        if tr_train_mask.sum().item() > 0:
            loss_tcl_pos = F.cross_entropy(tcl_pred.masked_select(tr_train_mask2).reshape([-1, 2]),
    ...
    def ohem(self, predict, target, train_mask):
        ...
        if n_pos.item() > 0:
            loss_pos = F.cross_entropy(predict.masked_select(pos2).reshape([-1, 2]),
                                        target.masked_select(pos).astype('int64'), reduction='sum')
            loss_neg = F.cross_entropy(predict.masked_select(neg2).reshape([-1, 2]),
                                        target.masked_select(neg).astype('int64'), reduction='none')
            n_neg = min(int(neg.astype('float32').sum().item()),
                        int(self.ohem_ratio * n_pos.astype('float32')),)
        else:
            loss_pos = torch.tensor(0., dtype='float32')
            loss_neg = F.cross_entropy(predict.masked_select(neg2).reshape([-1, 2]),
                                        target.masked_select(neg).astype('int64'), reduction='none')
            n_neg = 100
        if len(loss_neg) > n_neg:
            loss_neg, _ = torch.topk(loss_neg, k=n_neg)
    ...
    def fourier2poly(self, real_maps, imag_maps):
        ...
        imag_maps (tensor):A map composed of the imaginary parts of the Fourier coefficients, 
            whose shape is (-1, 2k+1)
        ...
        i_vect = torch.arange(0, self.num_samples, dtype='float32').reshape([1, -1])

        transform_matrix = 2 * np.pi / self.num_samples * torch.matmul(k_vect, i_vect)
