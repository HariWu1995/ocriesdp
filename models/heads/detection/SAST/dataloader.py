...
class Processor(object):
    ...
    def __init__(self, image_shape=[512, 512], min_crop_size=24, min_crop_side_ratio=0.3,
                                               min_text_size=10, max_text_size=512, **kwargs):
    ...
    def check_and_validate_polys(self, polys, tags, xxx_todo_changeme):
        ...
        for poly, tag in zip(polys, tags):
            ...
            if p_area > 0:
                ...
                poly = poly[tuple([0] + range(15, 0, -1)), :]
                quad = quad[tuple([0] + range( 3, 0, -1)), :]
    ...
    def crop_area(self, im, polys, tags, hv_tags, crop_background=False, max_tries=25):
        ...
        for i in range(max_tries):
            ...
            if xmax - xmin < self.min_crop_size \
    ...
    def generate_direction_map(self, poly_quads, direction_map):
        ...
        norm_width     = max(sum( width_list) / (len( width_list) + EPS), 1.0)
        average_height = max(sum(height_list) / (len(height_list) + EPS), 1.0)
        ...
        for quad in poly_quads:
            ...
            direction_label = tuple(
                map(float, [direct_vector[0], direct_vector[1], 1.0 / (average_height + EPS)]))
    ...
    def generate_tvo_and_tco(self, hw, polys, tags, tcl_ratio=0.3, ds_ratio=0.25):
        ...
        for poly, poly_tag in zip(polys, tags):
            ...
            cv2.fillPoly(poly_mask,
                         np.round(tcl_poly[np.newaxis, :, :]).astype(np.int32), 1)
    ...
    def __call__(self, data):
        ...
        # SAST head
        tvo_map, tco_map = self.generate_tvo_and_tco((self.input_size, self.input_size),
                                                      text_polys, text_tags, tcl_ratio=0.3, ds_ratio=0.25)
        ...
        data.update({
                    'image' : im_padded[::-1, :, :],
                  'tvo_map' :       tvo_map.transpose((2, 0, 1)),
                  'tco_map' :       tco_map.transpose((2, 0, 1)),
