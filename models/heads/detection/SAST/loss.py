...
class SASTLoss(nn.Module):
    ...
    def forward(self, predicts, labels):
        ...
        # score_loss
        intrs = torch.sum(f_score * l_score * l_mask)
        union = torch.sum(f_score * l_mask) + torch.sum(l_score * l_mask)
        score_loss = 1.0 - 2 * intrs / (union + 1e-5)
        ...
        border_loss = torch.sum(border_out_loss * l_border_score * l_border_mask) / \
                     (torch.sum(                  l_border_score * l_border_mask) + EPS)
        ...
        l_tvo_score      = l_score.expand(tvo_ex_shape)
        l_tvo_mask       = l_mask.expand(tvo_ex_shape)
        #
        tvo_geo_diff = l_tvo_split - f_tvo_split
        abs_tvo_geo_diff = torch.abs(tvo_geo_diff)
        tvo_sign = (abs_tvo_geo_diff < 1.0).float().detach()
        ...
        tvo_loss = torch.sum(tvo_out_loss * l_tvo_score * l_tvo_mask) / \
                  (torch.sum(               l_tvo_score * l_tvo_mask) + EPS)
        ...
        tco_sign = (abs_tco_geo_diff < 1.0).float().detach()
        ...
        tco_loss = torch.sum(tco_out_loss * l_tco_score * l_tco_mask) / \
                  (torch.sum(               l_tco_score * l_tco_mask) + EPS)
