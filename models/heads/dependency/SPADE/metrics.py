...
def cal_parsing_score(mode, parses, pr_parses, task, reformat, refine_parse=False, return_refined_parses=False,
                        allow_small_edit_distance=False, task_lan="", unwanted_fields=[],):
    get_init_stats_task = eval(f"get_init_stats_{task}")
    summary_task = eval(f"summary_{task}")

    if unwanted_fields:
        parses = trim_parse_for_scoring(parses, unwanted_fields, is_shallow=not reformat[0])
        pr_parses = trim_parse_for_scoring(pr_parses, unwanted_fields, is_shallow=not reformat[1])
    ...
    for pr1, gt1 in zip(pr, gt):
        if task in ["receipt_v1",]:
            out = get_statistics_receipt_v1(gt1, pr1,
                                            stats=stats,
    ...
    if return_refined_parses:
        return (f1, parse_stat, card_stat,
                corrects_parse, parses_refined, pr_parses_refined,)
    else:
...
def format_parses_receipt_v1(parses):
    ...
    for grouped_parse in parses:
        out = {"file_id": -1}
        new_parse = []
        for parses in grouped_parse:
            new_parse_temp = {}
            for parse in parses:
                k = get_key_from_single_key_dict(parse)
                v = parse[k]
                if k in new_parse_temp:
                    new_parse_temp[k] += [v]
                else:
                    new_parse_temp[k] = [v]
            new_parse.append(new_parse_temp)
