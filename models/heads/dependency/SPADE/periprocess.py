...
def collect_features_batchwise(features):
    data_ids, image_urls, texts, text_toks, text_tok_ids, labels, label_toks, \
        rn_center_toks, rn_dist_toks, rn_angle_toks, vertical_toks, char_size_toks, header_toks = [], [], [], [], [], [], [], [], [], [], [], [], []
    
    for x in features:
        data_ids.append(x["data_id"])
        image_urls.append(x["image_url"])
        texts.append(x["text"])
        text_toks.append(x["text_tok"])
        text_tok_ids.append(x["text_tok_id"])
        labels.append(x["label"])
        label_toks.append(x["label_tok"])
        rn_center_toks.append(x["rn_center_tok"])
        rn_dist_toks.append(x["rn_dist_tok"])
        rn_angle_toks.append(x["rn_angle_tok"])
        vertical_toks.append(x["vertical_tok"])
        char_size_toks.append(x["char_size_tok"])
        header_toks.append(x["header_tok"])

    return (data_ids, image_urls, texts, text_toks, text_tok_ids,
                                labels, label_toks,
                                    rn_center_toks, rn_dist_toks, rn_angle_toks,
                                     vertical_toks, char_size_toks, header_toks,)


def get_pretrained_transformer(path_data_folder, backbone_model_name, backbone_tweak_tag):
    backbone_path = Path(path_data_folder) / "model/backbones" / backbone_model_name / backbone_tweak_tag
    ...
    pretrained_transformer_config = func_config.from_pretrained(path_pretrained_transformer)
    ...
    count_model_weights(pretrained_transformer)

    return pretrained_transformer, pretrained_transformer_config


def check_consistency_between_backbone_and_encoder(pretrained_transformer_cfg, transformer_cfg):
    ...
    assert pretrained_transformer_cfg.intermediate_size == transformer_cfg.intermediate_size
    assert pretrained_transformer_cfg.num_attention_heads == transformer_cfg.num_attention_heads
    assert pretrained_transformer_cfg.max_position_embeddings == transformer_cfg.max_position_embeddings
    return None
...
def gen_split(feature, i_tok1, j_tok1, max_input_len):
    ...
    _ss0 = gen_slices(dim, 0, j_tok1 - i_tok1)
    _ss = gen_slices(dim, i_tok1, j_tok1)
...
def collect_output_of_target_layer(all_encoder_layer, encoder_layer_ids_used_in_decoder):
    encoded = []
    for i in encoder_layer_ids_used_in_decoder:
        encoded.append(all_encoder_layer[i])
    return torch.cat(encoded, dim=-1)  # [batch_size, max_input_len, dim]


def get_split_param_(text_tok, max_input_len, input_split_overlap_len, do_split=True):
    l_tok = len(text_tok)
...
def get_dim_of_id(id):
    if isinstance(id[0], Iterable):
        ...
        dim = len(id.shape)
        for d in id.shape:
            assert (d == id.shape[0])  # assume the length in each dimension is identical.
    else:
        dim = 1
...
class RelationTaggerUtils:
    ...
    @classmethod
    def save_analysis_results(cls, path_analysis_dir, test_type, test_score_dict,
                                parse_stat, card_stat, corrects_parse, parses, pr_parses,):
        os.makedirs(path_analysis_dir, exist_ok=True)

        write_json(path_analysis_dir / f"{test_type}_score_dict.jsonl", test_score_dict)
        write_json(path_analysis_dir / f"{test_type}_parse_stat.json", parse_stat)
        write_json(path_analysis_dir / f"{test_type}_card_stat.json", card_stat)

        write_jsonl(path_analysis_dir / f"{test_type}_correct_parse.jsonl", corrects_parse,)
        write_jsonl(path_analysis_dir / f"{test_type}_parses.jsonl", parses)
        write_jsonl(path_analysis_dir / f"{test_type}_pr_parses.jsonl", pr_parses)
    ...
    @classmethod
    def get_encoded1_part(cls, all_encoder_layer, max_input_len, input_split_overlap_len, n_seps,
                        i_sep, i_toks, j_toks, l_toks, encoder_layer_ids_used_in_decoder,):
        encoded1 = collect_output_of_target_layer(all_encoder_layer, encoder_layer_ids_used_in_decoder)  # [batch, max_input_len, dim]
    ...
    @classmethod
    def get_split_param(cls, text_toks, max_input_len, input_split_overlap_len, type_informer_tensor, do_split=True,):
        n_seps = []
        i_toks = []
        j_toks = []
        l_toks = []
        for text_tok in text_toks:
            n_sep, i_tok, j_tok, l_tok = get_split_param_(text_tok, max_input_len, input_split_overlap_len, do_split=do_split)
            n_seps.append(n_sep)
            i_toks.append(i_tok)
            j_toks.append(j_tok)
            l_toks.append(l_tok)
        ...
        i_toks = pad_ids(i_toks, max_length=max(n_seps), type_informer_tensor=type_informer_tensor)
        j_toks = pad_ids(j_toks, max_length=max(n_seps), type_informer_tensor=type_informer_tensor)
    ...
    def get_score(cls, model, batch_data_in, lmax_boxes):
        text_tok_ids, rn_center_toks, rn_dist_toks, rn_angle_toks, vertical_toks, \
            char_size_toks, header_toks, n_seps, i_toks, j_toks, l_toks = batch_data_in
        ...
        text_tok_ids, rn_center_x_toks, rn_center_y_toks, rn_dist_toks, rn_angle_toks, \
            vertical_toks, char_size_toks, header_toks = convert_feature_to_tensor(max(l_toks), text_tok_ids, 
                                                                                   rn_center_x_toks, rn_center_y_toks,
                                                                                   rn_dist_toks, rn_angle_toks,
                                                                                   vertical_toks, char_size_toks, header_toks,)

        n_seps, i_toks, j_toks, l_toks = convert_split_params_to_tensor(n_seps, i_toks, j_toks, l_toks)
        lmax_toks = l_toks.max().item()
        _b = len(n_seps)
        score = model(text_tok_ids, rn_center_x_toks, rn_center_y_toks, rn_dist_toks, rn_angle_toks,
                        vertical_toks, char_size_toks, header_toks,
                        n_seps, i_toks, j_toks, l_toks, lmax_toks, lmax_boxes,)
        return score

    @classmethod
    def generate_score_dict(cls, mode, avg_loss, p_edge, r_edge, f1_edge, f1, is_tensor=True):
        if is_tensor:
            score_dict = {
            ...
            for i, (p_e, r_e, f1_e) in enumerate(zip(p_edge, r_edge, f1_edge)):
                sub_score_dict = {
                    f"{mode}__precision_edge_of_type_{i}": p_e.item(),
                    f"{mode}__recall_edge_of_type_{i}": r_e.item(),
                    f"{mode}__f1_edge_of_type_{i}": f1_e.item(),
                }
            ...
            for i, (p_e, r_e, f1_e) in enumerate(zip(p_edge, r_edge, f1_edge)):
                sub_score_dict = {
                    f"{mode}__precision_edge_of_type_{i}": p_e,
                    f"{mode}__recall_edge_of_type_{i}": r_e,
                    f"{mode}__f1_edge_of_type_{i}": f1_e,
                }
                score_dict.update(sub_score_dict)
    ...
    def collect_outputs_gt(cls, task, outputs, return_aux=False):
        ...
        if task == "funsd" and return_aux:
            text_unit_field_labels = cls.gather_values_from_step_outputs(outputs, "text_unit_field_labels")
            f_parse_box_ids        = cls.gather_values_from_step_outputs(outputs, "f_parse_box_ids")
    ...
    def collect_outputs_pr(cls, task, outputs, return_aux=False):
        ...
        if task == "funsd" and return_aux:
            pr_text_unit_field_labels = cls.gather_values_from_step_outputs(outputs, "pr_text_unit_field_labels")
            pr_f_parse_box_ids        = cls.gather_values_from_step_outputs(outputs, "pr_f_parse_box_ids")
            return (pr_label_units, pr_parses, pr_text_unit_field_labels, pr_f_parse_box_ids,)
        else:
    ...
    def gather_values_from_step_outputs(cls, outputs, key):
        # each_step / batch
        # [combine each batch results]
        return reduce(lambda accum, results: accum + results, [x[key] for x in outputs], [],)
    ...
    def cal_f1_scores(cls, task, mode, parses1, parses2, parse_refine_options):
        if parses1[0] is None:
            ...
            if task == "funsd":
                ...
                f1, parse_stat, card_stat, corrects_parse = cal_parsing_score(
                    mode,
                    parses1,
                    parses2,
                    task,
                    ...
                    allow_small_edit_distance=parse_refine_options["allow_small_edit_distance"],
    ...
    @classmethod
    def count_tp_fn_fp(cls, task, label_units, pr_label_units, mode, parses, pr_parses, parse_refine_options,):

        tp_edge, fn_edge, fp_edge = cal_tp_fn_fp_of_edges(label_units, pr_label_units)
        _, parse_stat, _, _ = cls.cal_f1_scores(task, mode, parses, pr_parses, parse_refine_options)
