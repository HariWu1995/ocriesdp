"""
Task: KIE - Key Information Extraction

Spatial Dual-Modality Graph Reasoning for Key Information Extraction
    Paper: https://arxiv.org/pdf/2103.14470.pdf
    Code: https://github.com/PaddlePaddle/PaddleOCR/blob/release/2.5/ppocr/modeling/heads/kie_sdmgr_head.py
"""
import sys
from typing import *
...
class GNNLayer(nn.Module):
    ...
    def forward(self, nodes, edges, nums):
        ...
        for num in nums:
            ...
            cat_nodes.append(
                torch.concat([sample_nodes.unsqueeze(1).expand(-1, num, -1),
                              sample_nodes.unsqueeze(0).expand(num, -1, -1),], -1).reshape([num**2, -1]))
            start += num
...
class FusionBlock(nn.Module):
    ...
    def __init__(self, input_dims, output_dim, mm_dim=1600, chunks=20, rank=15,
                 shared=False, dropout_input=0., dropout_pre_lin=0., dropout_output=0., pos_norm='before_cat'):
        super().__init__()
        self.rank = rank
    ...
    def forward(self, x):
        ...
        x0_chunks = torch.split(x0, split_size_or_sections=self.chunks, dim=-1)
        x1_chunks = torch.split(x1, split_size_or_sections=self.chunks, dim=-1)
        zs = []
...
class SDMGRHead(nn.Module):
    ...
    def __init__(self, in_channels, num_chars=92, visual_dim=16, fusion_dim=1024,
                       node_input=32, node_embed=256,
                       edge_input=5, edge_embed=256, num_gnn=2, num_classes=26, bidirectional=False):
        super().__init__()

        self.fusion = FusionBlock([visual_dim, node_embed], node_embed, fusion_dim)
        self.node_embed = nn.Embedding(num_chars, node_input, 0)
        hidden_dim = node_embed // 2 if bidirectional else node_embed
        self.rnn = nn.LSTM(input_size=node_input, hidden_size=hidden_dim, num_layers=1)
        self.edge_embed = nn.Linear(edge_input, edge_embed)
        self.reasoner = nn.ModuleList([GNNLayer(node_embed, edge_embed) for _ in range(num_gnn)])
    ...
    def forward(self, input, targets):
        ...
        all_nodes = torch.concat([
                    torch.concat([text, torch.zeros((text.shape[0], max_num-text.shape[1]))], -1) for text in texts])
        ...
        temp_all_nums = temp_all_nums.expand(-1, -1, rnn_nodes.shape[-1])
        ...
        for gnn_layer in self.reasoner:
