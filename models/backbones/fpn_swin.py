...
class FPN(Backbone):
    ...
    def __init__(self, bottom_up, in_features, out_channels, norm="", top_block=None, fuse_type="sum"):
        ...
        for idx, in_channels in enumerate(in_channels_per_feature):
            ...
            # output_conv = Conv2d(out_channels, out_channels, kernel_size=3, stride=1, padding=1,
            #                       bias=use_bias, norm=output_norm, )
            output_conv = SwinTransformerStage(dim=out_channels, depth=1, num_heads=2, window_size=7)
    ...
    def forward(self, x):
        ...
        Returns:
            dict[str->Tensor]:
                mapping from feature map name to FPN feature map tensor in high to low resolution order. 
                Returned feature names follow the FPN paper convention: "p<stage>", where stage has stride = 2 ** stage 
                    e.g., ["p2", "p3", ..., "p6"].
        """
