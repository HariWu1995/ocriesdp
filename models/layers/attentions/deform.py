...
class MultiScaleDeformableAttentionFunction(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        grad_value, grad_sampling_loc, grad_attn_weight = _C.ms_deform_attn_backward(value, 
                value_spatial_shapes, value_level_start_index, sampling_locations, attention_weights, grad_output, ctx.im2col_step)
...
class MultiScaleDeformableAttention(nn.Module):
    ...
    def _reset_parameters(self):
        ...
        thetas = torch.arange(self.n_heads, dtype=torch.float32) * (2.0 * PI / self.n_heads)
        grid_init = torch.stack([thetas.cos(), 
                                 thetas.sin(),], dim=-1)
    ...
    def forward(self, query, reference_points, input_flatten, input_spatial_shapes, input_level_start_index, input_padding_mask=None):
        ...
        output = MultiScaleDeformableAttentionFunction.apply(value, input_spatial_shapes, input_level_start_index, sampling_locations, attention_weights, self.im2col_step)
