...
from models.necks.layers import Sum, Parallel, Interpolate
...
class PanopticFPN(nn.Sequential):
    ...
    The feature maps are passed through the architecture shown below, producing a single final output, with out_channels channels.
    ...
    1st feat. map ────[1st in_conv]─────────>───[1st upsampler]─────────(+)
                                                                        ...
                                                                        out
    """
    def __init__(self, in_feats_shapes: Sequence[Tuple[int, ...]],
        ...
        in_convs = Parallel([
            nn.Conv2d(s[1], hidden_channels, kernel_size=1) for s in in_feats_shapes
        ])
        upsamplers = self._make_upsamplers(in_channels=hidden_channels,
                                            size=out_size,
        ...
        # yapf: disable
        layers = [in_convs, upsamplers, Sum(), out_conv]
        # yapf: enable
        super().__init__(*layers)

    @classmethod
    def _make_upsamplers(cls, in_channels: int, size: int,
    ...
    @classmethod
    def _upsample_feat(cls, in_channels: int, num_upsamples: int,
                       size: int, scale_factor: float = 2., num_groups: int = 32) -> nn.Sequential:
        if num_upsamples == 0:
            return cls._make_upsampling_block(in_channels=in_channels,
                                             out_channels=in_channels // 2, scale=1, num_groups=num_groups)
        blocks = []
        for _ in range(num_upsamples - 1):
            blocks.append(
                cls._make_upsampling_block(in_channels=in_channels,
    ...
    @classmethod
    def _make_upsampling_block(cls, in_channels: int, out_channels: int = None,
                               scale: float = 2, size: int = None, num_groups: int = 32) -> nn.Sequential:
